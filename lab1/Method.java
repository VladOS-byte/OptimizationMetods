...
public abstract class Method {
	...
	Method(Function<Double, Double> function) {
	 ...
	 * @see #minimize
	 */
	public abstract double minimize(List<Point<Double>> series, double leftBorder, double rightBorder);
	 ...
	 */
	public void addPointToSeries(List<Point<Double>> series, double x, double y, int key) {
		if (Double.compare(x, Double.NaN) == 0 || Double.isInfinite(x) ||
				Double.compare(y, Double.NaN) == 0 || Double.isInfinite(y)) {
			return;
		}
		series.add(new Point<>(x, y, key));
	}
	 ...
	 * @param series - <code>series</code> of points
	 */
	public void clear(List<Point<Double>> series) {
		series.clear();
		key = 0;
	}
