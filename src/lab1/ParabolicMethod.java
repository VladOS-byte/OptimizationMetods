...
public class ParabolicMethod extends OneDimensionMethod {
	...
	public double minimize(List<StepFrame<Double>> series, double leftBorder, double rightBorder) {
        ...
        addPointToSeries(series, leftBorder, fl, key++, leftBorder, rightBorder);
        addPointToSeries(series, rightBorder, fr, key++, leftBorder, rightBorder);
        ...
        addPointToSeries(series, m1, fm1, key++, leftBorder, rightBorder);
        return m1;
	}
    ...
    private Point<Double> chooseMiddle(List<StepFrame<Double>> series, double left, double right, double fl, double fr) {
    	double fme = function.apply(left + epsilon);
    	addPointToSeries(series, left + epsilon, fme, key++, left, right);
        if (fme <= fl) {
            return new Point<>(left + epsilon, fme);
        }
        fme = function.apply(right - epsilon);
    	addPointToSeries(series, right - epsilon, fme, key++, left, right);
        if (fme <= fr) {
            return new Point<>(right - epsilon, fme);
        }
